function PlayData() {
	...
	PlayData.prototype.toSimpleString = function() {
	    if(this.isPlay()){
		return this.valueText + '/' + this.wealth.toFixed(8);
	    }else{
		return '未投注';
	    }
	};
	...
	PlayData.prototype.toReviewString = function() {
	    ...
	    review = review.replaceAll('{wealth}', this.wealth.toFixed(8));
...
var GuessPlayHelper = {
	...
	var playWealth = parseFloat(parseFloat(playWealthInput.val()));
	...
	playData.setWealth(parseFloat(parseFloat($('#play_wealth_' + playWayId).val()).toFixed(8)));
	if(!playData.isPlay()){
	    // 删除总计部分
	...
	$('#total_wealth').html(this.totalWealth().toFixed(8));
