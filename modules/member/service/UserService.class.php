...
class UserService extends TransationSupport implements IUserService
{
    ...
    public function register(User $user)
    {
        ...
        $user->setAvailableBtc(0.0);
        $user->setRegisterTime(time());
        $user->setbtcAddress("Address-tmp" . time());
    ...
    {
        if (! is_numeric($code) || $code <= self::$baseNumber)
            return 0;
    ...
    /*
     * @see IUserService::btc()
     */
    public function btc($io, $freezeCode = 0)
    {
        if (! $io)
            return false;
        if ($freezeCode) {
            if ($freezeCode > 0) {
                ...
                $update = array(
                    ...
                    'freeze_btc' => "freeze_btc - {$io['wealth']}"
                );
            } else {
                // 冻结
                $userId = $io['from_user_id'];
     ...
     */
    public function handsel(User $fromUser, $toUser, $btc)
    {
        if (! $fromUser || ! $toUser || $btc < 1 || $fromUser->getAvailableBtc() < $btc) {
            return false;
        }
        try {
            $this->beginTransation();
            $io = array(
                ...
                'from_balance' => $fromUser->getAvailableBtc() - $btc,
                'to_user_id' => $toUser['id'],
                'to_title' => "用户[{$fromUser->getName()}]赠送",
                'to_balance' => $toUser['available_btc'] + $btc,
    ...
    public function apply_friend($uid1, $uid2, $type)
    {
        ...
        if ($type == 1) // 添加好友
{
            if (strpos(',' . $u1_friend . ',', ',' . $uid2 . ',') === false)
                $u1_friend = empty($u1_friend) ? $uid2 : $u1_friend . ',' . $uid2;
            if (strpos(',' . $u2_friend . ',', ',' . $uid1 . ',') === false)
                $u2_friend = empty($u2_friend) ? $uid1 : $u2_friend . ',' . $uid1;
        } else // 取消好友
{
