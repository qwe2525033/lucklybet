...
class GuessService extends TransationSupport implements IGuessService
{
    ...
    public function add(Guess $guess)
    {
        ...
        try {
            ...
            // 更新竞猜点的竞猜数
            $guessPointDao = MD('GuessPoint');
            if (! $guessPointDao->update(array(
                'guess_count' => 'guess_count+1'
            ), $guess->getGuessPointId(), true)) {
                $this->rollBack();
                return false;
            }
            ...
            if ($keepWealth) {
                ...
                if ($guess->wealthTypeIsBtc()) {
                    $io = array(
                        ...
                        'wealth' => $keepWealth,
                        'from_balance' => $user->getAvailableBtc() - $keepWealth
                    );
    ...
    public function delete(Guess $guess)
    {
        ...
        try {
            ...
            if ($guess->getStatus() != Guess::STATUS_WAITING_CKECK) {
                // 更新用户坐庄竞猜个数
                $userDao = MD('User');
                if (! $userDao->update(array(
                    'guess_add_count' => 'guess_add_count-1'
                ), $guess->getUserId(), true)) {
                    $this->rollBack();
                    return false;
                }
            }
                ...
                if (! $guess->getCustom() && $keepWealth) {
                    ...
                    if ($guess->wealthTypeIsBtc()) {
                        $io = array(
                            ...
                            'wealth_type' => Io::WEALTH_TYPE_MONEY,
                            'wealth' => $keepWealth,
                            'to_balance' => $user->getAvailableBtc() + $keepWealth
                        );
                        if (! $userService->btc($io, 1)) {
                            $this->rollBack();
                            return false;
    ...
    {
        $userGuessService = GuessServiceFactory::getUserGuessService();
        return $userGuessService->count(array(
            'type' => UserGuess::TYPE_ATTENTION,
            'from_uid' => $userId,
            'guess_id' => $guessId
        ));
    ...
    {
        if (! $guess || $guess->getStatus() != Guess::STATUS_WAITING_CKECK)
            return false;
        try {
            $this->beginTransation();
            // 改变状态
            $success = MD('Guess')->update(array(
                'status' => Guess::STATUS_NORMAL
            ), $guess->getId());
            if (! $success) {
                $this->rollBack();
                return false;
...
//
                ...
                if ($guess->wealthTypeIsBtc()) {
                    ...
                    $io['to_balance'] = $guessUser->getAvailableBtc() + $io['wealth'];
                    
                    // 更新金额
                    $update = array(
                        'available_btc' => "available_btc + {$io['wealth']}",
                        'freeze_btc' => "freeze_btc - {$guessKeepWealth}"
                    );
                ...
                // 更新竞猜状态
                if (! $guessDao->update(array(
                    'status' => Guess::STATUS_END,
                    'win_wealth' => $guessWinWealth
                ), $guess->getId())) {
                    $this->rollBack();
                    return false;
                }
            ...
            // 更新竞猜点的状态
            if (! MD('GuessPoint')->update(array(
                'status' => GuessPoint::STATUS_RUDGED
            ), $guessPoint->getId())) {
                $this->rollBack();
                return false;
            }
    ...
    {
        $guessDao = MD('Guess');
        $tempGuesses = $guessDao->gets(array(
            'guess_point_id' => $guessPoint->getId()
        ));
