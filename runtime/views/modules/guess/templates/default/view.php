...
<script type="text/javascript" src="/res/js/jquery/jquery.scrollTo.min.js"></script>
<script type="text/javascript" src="/res/js/play.js"></script>
			...
			<form id="ajaxform" name="ajaxform" action="/guess/play/?inajax=1" method="post">
				...
				<div class="h">
					...
					<?php if($playData->isFloatOdds()){ ?>
						<div class="w2">浮动赔率</div>
					<?php }elseif($playData->isFixedOdds()){ ?>
						<div class="w2">固定赔率</div>
					<?php }else{ ?>
						<div class="w2">赔率</div>
					<?php } ?>
				...
				<div id="play_box_<?php echo $playData->getId(); ?>" class="play_box">
				    ...
				    <div class="control-group">
					    ...
					    <div class="controls">
					    	<input type="text" id="play_wealth_<?php echo $playData->getId(); ?>" onkeyup="GuessPlayHelper.checkWealth('<?php echo $playData->getId(); ?>')" name="play[<?php echo $playData->getId(); ?>][wealth]" class="span3" min="<?php echo $playData->getBettingLowerLimit(); ?>" max="<?php echo $playData->getBettingUpperLimit(); ?>" <?php if($playData->getBettingLowerLimit()){ ?>value="<?php echo $playData->getBettingLowerLimit(); ?>"<?php }else{ ?>value="0.00000000"<?php } ?>> 
					    	<span style="margin-left: 15px">
					    	0为不投注或取消投注&nbsp;
				...
				<div class="tzinfo">
					...
					<div class="i">
						<?php if($guess->wealthTypeIsBtc()){ ?>
						<p>莱特币：<span><?php echo $user->getAvailableBtc(); ?></span></p>
